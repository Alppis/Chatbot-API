...
KEYWORD2 = {'keyword': KEYWORD2_ID,
        'response1': 'Linux is best!',
        'response2': 'Debian or Red Hat?',
        'header': 'Shebang',
...
class KeywordTestCase(unittest.TestCase):
    ...
    def test_create_keyword_object(self):
        '''
        Checks that method _create_keyword_object works returning adequate response
        '''
        ...
        with con:
            ...
            row = cur.fetchone()
        #Test the method
        response = self.connection._create_keyword_object(row)
        self.assertDictContainsSubset(response, KEYWORD1)
